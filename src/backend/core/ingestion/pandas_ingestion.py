...
class PandasIngestion(DataIngestionBase):
    ...
    def _load_csv(self, file_path: str, use_chunking: bool, chunk_size: int, **kwargs) -> pd.DataFrame:
        ...
        # Ensure engine is set to pyarrow for better performance with large files
        if 'engine' not in kwargs:
            kwargs['engine'] = 'pyarrow'
        ...
        for chunk in pd.read_csv(file_path, chunksize=chunk_size, **kwargs):
            if not chunks:  # First chunk
                # Infer optimal dtypes from the first chunk
    ...
    def _load_parquet(self, file_path: str, use_chunking: bool = False, chunk_size: int = 100000, **kwargs) -> pd.DataFrame:
        """
        ...
        Returns:
            Loaded pandas DataFrame with optimized memory usage
        """
        # First determine file size to choose appropriate loading strategy
        file_size = os.path.getsize(file_path)
        ...
        # For large files, use chunking with PyArrow
        # Note: pd.read_parquet doesn't support chunksize directly, so we use filters
        # to read the file in chunks based on row ranges

        # First, get the number of rows in the parquet file
        # This is a lightweight operation that doesn't load the full data
        metadata = pd.read_parquet(file_path, engine='pyarrow').shape[0]
        total_rows = metadata
