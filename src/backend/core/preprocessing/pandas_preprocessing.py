...
from core.preprocessing.utils.common_operations import (
    get_operation_handler, get_available_operations as get_common_operations,
    process_operations, is_numeric_dtype, is_categorical_dtype, handle_all_columns
)
...
class PandasPreprocessing(PreprocessingBase):
    ...
    def process(self, data: pd.DataFrame, operations: List[Dict[str, Any]]) -> pd.DataFrame:
        ...
        # Define a function to apply a single operation
        def apply_single_operation(data: pd.DataFrame, operation: Dict[str, Any]) -> pd.DataFrame:
            op_type = operation.get("type")
            params = operation.get("params", {})
            logger.info(f"Applying operation: {op_type} with params: {params}")

            try:
                # Get the method that handles this operation type
                handler_name = get_operation_handler(op_type)
                handler_method = getattr(self, handler_name)

                # Apply the operation
                result = handler_method(data, **params)
        ...
        # Use the common process_operations function
        return process_operations(result, operations, apply_single_operation)
    ...
    def is_operation_valid(self, data: pd.DataFrame, operation: Dict, operation_history: List[Dict]) -> bool:
        ...
        # Default to true for other operations
        return True
    ...
    def get_available_operations(self) -> Dict[str, Dict[str, Any]]:
        ...
        """
        # Use the common operations dictionary
        return get_common_operations()

    def _drop_columns(self, data: pd.DataFrame, columns: List[str]) -> pd.DataFrame:
        """Drop specified columns from the DataFrame."""
        # Use the common handle_all_columns function
        columns_to_drop = handle_all_columns(columns, data.columns)
    ...
    def _fill_missing(self, data: pd.DataFrame, columns: Union[List[str], str],
        ...
        # Determine which columns to process using common function
        target_columns = handle_all_columns(columns, data.columns)
    ...
    def _encode_categorical(self, data: pd.DataFrame, columns: List[str],
                       method: str = "one_hot", target_column: Optional[str] = None) -> pd.DataFrame:
        """Encode categorical variables."""
        result = data.copy()

        # Handle 'all' selection using common function
        if "all" in columns:
            columns = [
                col for col in handle_all_columns(columns, data.columns)
                if (
        ...
        if method == "one_hot":
            ...
            for col in columns:
                ...
                reverse_mapping = {int(new_idx): old_idx for old_idx, new_idx in enumerate(random_order)}
    ...
    def _scale_numeric(self, data: pd.DataFrame, columns: List[str],
        ...
        # Process columns using common function
        columns_to_process = handle_all_columns(columns, data.columns)
